...
class OciResponseIterator implements Iterator
{
    ...
    /**
     * Whether the operation being called is an async operation or not
     */
    /*bool*/ private $isAsyncOperation = false;
    ...
    public function __construct(
        ...
        $this->isAsyncOperation = (substr($operation, -5) == 'Async');
...
class OciItemIterator implements Iterator
{
    ...
    /**
     * Build a function that returns the items in the specified response body property.
     * This is a special helper method for the Object Storage listObjects operation that combines
     * the results from 'objects' and 'prefixes'.
     * @param null $propertyName not used
     * @return callable function to extract the items
     */
    public static function buildResponseItemsGetterForObjectStorageListObjects($propertyName)
    {
        return function ($response) {
            ...
            $objects = null;
            if (property_exists($response->getJson(), 'objects')) {
                $objects = $response->getJson()->objects;
            }
            $prefixes = null;
            if (property_exists($response->getJson(), 'prefixes')) {
                $prefixes = $response->getJson()->prefixes;
            }
            if (!is_array($objects)) {
                throw new InvalidArgumentException("Expected an array for 'objects', received " . StringUtils::get_type_or_class($objects));
            }
            if ($prefixes) {
                if (!is_array($prefixes)) {
                    throw new InvalidArgumentException("Expected an array for 'prefixes', received " . StringUtils::get_type_or_class($prefixes));
                }
                foreach ($prefixes as $p) {
                    $objects[] = json_decode("{\"name\":\"$p\",\"isPrefix\":\"true\"}");
                }
            }
            return $objects;
        };
