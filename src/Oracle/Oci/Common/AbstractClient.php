...
abstract class AbstractClient
{
    ...
    protected function setClientWithDefaultHandler()
    {
        ...
        $this->client = new Client([
            'handler' => $this->stack,
            'stream' => true,
            'debug' => $this->logger()->scope("middleware\\guzzle\\verbose")->isDebugEnabled()
        ]);
    }
    ...
    /**
     * Get the middleware stack. Primarily for mock testing. Use with care.
     * @return HandlerStack
     */
    ...
    /**
     * Get the region, if one was set.
     * @return Region|null region, or null if none was set
     */
     ...
     * @return AuthProviderInterface auth provider
     */
    ...
    protected function signingMiddleware(RequestInterface $request)
    {
        ...
        $middlewareLogger->debug("Request URI: " . $request->getMethod() . " ". $request->getUri(), "uri");
    ...
    private function sendRequestAsync(&$request, $responseBodyType)
    {
        $responsePromise = $this->client->sendAsync($request)->then(
            function ($__response) use ($responseBodyType) {
                ...
                $this->logger("response")->debug("OciResponse: $r");
                return $r;
            },
            function ($__e) {
                $obre = HttpUtils::processBadResponseException($__e);
                $this->logger("response")->debug("OciBadResponseException: $obre");
                throw $obre;
            }
        );
