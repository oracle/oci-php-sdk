...
{
    /**
     * Log names matching these regular expressions will be forced to the specified log level
     * (0 to disable completely) unless the log names are specifically overridden, or a different
     * set of regexes is passed.
     */
    const DEFAULT_LOG_NAME_REGEXES = [
        ...
        '/\\\\verbose/' => 0,
        '/^verbose/' => 0,
        '/\\\\X509FederationClient\\\\response/' => 0
    ];
    ...
    public function __construct(
        ...
        if ($logNameRegexes !== null) {
            $this->logNameRegexes = $logNameRegexes;
        } else {
            $this->logNameRegexes = AbstractLogAdapter::DEFAULT_LOG_NAME_REGEXES;
        }
    }
